$(document).ready(() => {
	...
	let totalPrice = 0;
	const $totalElement = $('<div id="total"></div>')
		.text(
	...
	function checkerInfo() {
		$("#form").on('submit', (e) => {
			e.preventDefault();
			$('.msg').remove();
			if($enteredUserName.val() === userLoginInfo.userName && $enteredUserPassword.val() === userLoginInfo.password) {
				$('#login-form').hide();
				$menu.show()
				$order.show()
			} else if ($enteredUserName.val() !== userLoginInfo.userName && $enteredUserPassword.val() !== userLoginInfo.password) {
				$('.errorMessage').append(`<p class='msg'>Your entered username: ${$enteredUserName.val()} can not be found, and your password: ${$enteredUserPassword.val()} is invalid</p>`)
			...
			checkerInfo();
				...
				importantInfoArray.forEach((foodSetObj, foodSetIndex) => {
					const $foodBox = $("<div class='foodBox card'></div>").text(`${foodSetObj.name}`).css('border', 'solid');
					$foodBox.append($("<div class='price'></div>").text(`Price $${foodSetObj.price}`));
					$foodBox.append($("<div class='include'></div>").text(`Includes: ${foodSetObj.details}`));
					
					const $foodImage = $(`<img class="foodImage" id="${foodSetObj.name}-image" src=${foodSetObj.imgUrl}>`);
				...
				function $getAddButton(foodSetObj, foodSetIndex) {
					return $("<button class='btn button'>Add Item</button>")
					.on('click', () => {
